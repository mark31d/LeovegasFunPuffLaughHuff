...
function TabIcon({ icon, active, onPress, isHome = false }) {
  // Иконка дома черная когда НЕ активна, остальные белые при активности
  const getTintColor = () => {
    if (isHome) {
      return active ? '#FFFFFF' : '#000000'; // иконка дома: белая если активна, черная если неактивна
    }
    return active ? '#FFFFFF' : '#1C1C1C';
  };
